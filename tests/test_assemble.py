...
class TestAssembler(unittest.TestCase):
    ...
    def test_assemble_random(self):
        ...
        r = v2f(r_arr_in, fs=worm.PDE.function_spaces['r'])
        theta = v2f(theta_arr_in, fs=worm.PDE.function_spaces['theta'])

        worm._update_inputs(eps0_arr_in, k0_arr_in)
        worm.assembler.update_state(r, theta, t_in)
        worm.assembler.assemble_output()
        worm.assembler.assemble_input()
    ...
    def test_assemble_input_expression(self):
        ...
        for _ in range(2):
            ...
            eps0 = Expression(('cos(x[0]+t)', 'sin(x[0]+t)', '0'), degree=worm.numerical_param.fed, t=0.0)
            k0 = Expression(('sin(x[0]+t)', 'cos(x[0]+t)', '0'), degree=worm.numerical_param.fed, t=0.0)
